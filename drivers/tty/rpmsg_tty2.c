...
static ssize_t tty_rpmsg_write(struct tty_struct *tty,
...
static int rpmsg_tty_match(struct rpmsg_device *dev, const struct rpmsg_driver *drv)
{
	 ...
	 * except for those who are exactly rpmsg-tty.
	 * They belong to the original rpmsg-tty driver
	 */
	if (strlen(devname) == sizeof(drvname) - 1)
		return 0;

	return strncmp(devname, drvname, sizeof(drvname) - 1) == 0;
}
