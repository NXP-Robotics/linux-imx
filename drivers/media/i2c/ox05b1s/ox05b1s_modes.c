// SPDX-License-Identifier: GPL-2.0-only
/*
 * Register configurations for all sensor supported modes
 * Copyright 2024-2025, NXP
 * Copyright 2024, Omnivision
 * Copyright 2024, Verisilicon
 *
 */

#include <media/v4l2-cci.h>
#include "ox05b1s.h"

#define OS08A20_REG_PLL_CTRL_05		CCI_REG8(0x0305)
#define OS08A20_REG_PLL_CTRL_25		CCI_REG8(0x0325)
#define OS08A20_REG_MIPI_BIT_10_12	CCI_REG8(0x031e)
/* Analog Control Registers 0x3600-0x3637 */
#define OS08A20_REG_ANA_CTRL		CCI_REG8(0x3600)
#define OS08A20_REG_CORE_0		CCI_REG8(0x3660)
/* Sensor Timing Control Registers 0x3700-0x37ff */
#define OS08A20_REG_SENSOR_TIMING_CTRL	CCI_REG8(0x3700)
#define OS08A20_REG_L2S_MODE		CCI_REG8(0x3797)
#define OS08A20_REG_X_ODD_INC		CCI_REG8(0x3814)
#define OS08A20_REG_Y_ODD_INC		CCI_REG8(0x3816)
#define OS08A20_REG_FORMAT1		CCI_REG8(0x3820)
#define OS08A20_REG_FORMAT2		CCI_REG8(0x3821)
#define OS08A20_REG_BLC_CTRL08		CCI_REG8(0x4008)
#define OS08A20_REG_BLC_CTRL09		CCI_REG8(0x4009)
#define OS08A20_REG_BLC_CTRL50		CCI_REG8(0x4050)
#define OS08A20_REG_BLC_CTRL51		CCI_REG8(0x4051)
#define OS08A20_REG_PCLK_PERIOD		CCI_REG8(0x4837)
#define OS08A20_REG_ISP_CTRL_1		CCI_REG8(0x5001)
#define OS08A20_REG_ISP_CTRL_5		CCI_REG8(0x5005)

/* Common register configuration for Omnivision OS08A20 raw camera */
static const struct cci_reg_sequence os08a20_init_setting_common[] = {
	{OS08A20_REG_ANA_CTRL + 0x05, 0x50},
	{OS08A20_REG_ANA_CTRL + 0x10, 0x39},
	{OS08A20_REG_SENSOR_TIMING_CTRL + 0x5e, 0x0b},
	{OS08A20_REG_L2S_MODE, 0x04},
	{OS08A20_REG_BLC_CTRL08, 0x02},
	{OS08A20_REG_BLC_CTRL09, 0x0d},
	{OS08A20_REG_BLC_CTRL50, 0x00},
	{OS08A20_REG_BLC_CTRL51, 0x01},
	{OS08A20_REG_ISP_CTRL_1, 0x42},
	{OS08A20_REG_ISP_CTRL_5, 0x00},
};

/* Common register configuration for Omnivision OS08A20 10 bit */
static const struct cci_reg_sequence os08a20_init_setting_10bit[] = {
	{OS08A20_REG_MIPI_BIT_10_12, 0x09},
	{OS08A20_REG_ANA_CTRL + 0x09, 0xb5},
	{OS08A20_REG_CORE_0, 0x43},
	{OS08A20_REG_SENSOR_TIMING_CTRL + 0x06, 0x35},
	{CCI_REG24(0x3709), 0x490098},
};

/* Common register configuration for Omnivision OS08A20 12 bit */
static const struct cci_reg_sequence os08a20_init_setting_12bit[] = {
	{OS08A20_REG_MIPI_BIT_10_12, 0x0a},
	{OS08A20_REG_ANA_CTRL + 0x09, 0xdb},
	{OS08A20_REG_CORE_0, 0xd3},
	{OS08A20_REG_SENSOR_TIMING_CTRL + 0x06, 0x6a},
	{CCI_REG24(0x3709), 0x480130},
};

/* Mode specific register configurations for Omnivision OS08A20 raw camera */

/* OS08A20 3840 x 2160 @30fps BGGR10 */
static const struct cci_reg_sequence os08a20_init_setting_4k_10b[] = {
	{OS08A20_REG_PLL_CTRL_05, 0x3c},
	{OS08A20_REG_FORMAT2, 0x04}, /* mirror */
	{OS08A20_REG_PCLK_PERIOD, 0x10},
};

/* OS08A20 3840 x 2160 @30fps BGGR12 */
static const struct cci_reg_sequence os08a20_init_setting_4k_12b[] = {
	{OS08A20_REG_PLL_CTRL_05, 0x3c},
	{OS08A20_REG_PLL_CTRL_25, 0x47},
	{OS08A20_REG_FORMAT2, 0x04}, /* mirror */
	{OS08A20_REG_PCLK_PERIOD, 0x10},
};

/* OS08A20 1920 x 1080 @60fps BGGR10 */
static const struct cci_reg_sequence os08a20_init_setting_1080p_10b[] = {
	{OS08A20_REG_PLL_CTRL_05, 0x2d},
	{OS08A20_REG_PLL_CTRL_25, 0x45},
	{OS08A20_REG_X_ODD_INC, 0x03},
	{OS08A20_REG_Y_ODD_INC, 0x03},
	{OS08A20_REG_FORMAT1, 0x01}, /* vertical bining */
	{OS08A20_REG_FORMAT2, 0x05}, /* mirror, horizontal bining */
	{OS08A20_REG_BLC_CTRL09, 0x05},
	{OS08A20_REG_PCLK_PERIOD, 0x16},
};

const struct ox05b1s_reglist os08a20_reglist_4k_10b[] = {
	{
		.regs = os08a20_init_setting_common,
		.count = ARRAY_SIZE(os08a20_init_setting_common),
	},
	{
		.regs = os08a20_init_setting_10bit,
		.count = ARRAY_SIZE(os08a20_init_setting_10bit),
	},
	{
		.regs = os08a20_init_setting_4k_10b,
		.count = ARRAY_SIZE(os08a20_init_setting_4k_10b),
	},
	{
		/* sentinel */
	}
};

const struct ox05b1s_reglist os08a20_reglist_4k_12b[] = {
	{
		.regs = os08a20_init_setting_common,
		.count = ARRAY_SIZE(os08a20_init_setting_common),
	},
	{
		.regs = os08a20_init_setting_12bit,
		.count = ARRAY_SIZE(os08a20_init_setting_12bit),
	},
	{
		.regs = os08a20_init_setting_4k_12b,
		.count = ARRAY_SIZE(os08a20_init_setting_4k_12b),
	},
	{
		/* sentinel */
	}
};

const struct ox05b1s_reglist os08a20_reglist_1080p_10b[] = {
	{
		.regs = os08a20_init_setting_common,
		.count = ARRAY_SIZE(os08a20_init_setting_common),
	},
	{
		.regs = os08a20_init_setting_10bit,
		.count = ARRAY_SIZE(os08a20_init_setting_10bit),
	},
	{
		.regs = os08a20_init_setting_1080p_10b,
		.count = ARRAY_SIZE(os08a20_init_setting_1080p_10b),
	},
	{
		/* sentinel */
	}
};

/*
 * OS08A20 3840 x 2160 @60fps BGGR10, 4-lane MIPI 1440 Mbps/lane.
 * Full sensor register sequence ported from the OmniVision/Axera reference
 * (AXERA ax650n_bsp_sdk ov_os08a20 "Linear10 60fps MCLK24M"), which is the
 * validated 60fps clock/timing recipe. Differs from the 30fps mode mainly in
 * the PLL (0x0305=0x5a), line length (HTS 0x040c) and MIPI PHY timing; the
 * frame rate vs the 30fps sibling is set by VTS (0x090a here vs 0x1214).
 * Stream on/off (0x0100) and soft-reset (0x0103) are handled by the driver
 * and were intentionally omitted.
 */
static const struct cci_reg_sequence os08a20_init_setting_4k60_10b[] = {
	{CCI_REG8(0x0303), 0x01},
	{CCI_REG8(0x0305), 0x5a},
	{CCI_REG8(0x0306), 0x00},
	{CCI_REG8(0x0308), 0x03},
	{CCI_REG8(0x0309), 0x04},
	{CCI_REG8(0x032a), 0x00},
	{CCI_REG8(0x300f), 0x11},
	{CCI_REG8(0x3010), 0x01},
	{CCI_REG8(0x3011), 0x04},
	{CCI_REG8(0x3012), 0x41},
	{CCI_REG8(0x3016), 0xf0},
	{CCI_REG8(0x301e), 0x98},
	{CCI_REG8(0x3031), 0xa9},
	{CCI_REG8(0x3103), 0x92},
	{CCI_REG8(0x3104), 0x01},
	{CCI_REG8(0x3106), 0x10},
	{CCI_REG8(0x3400), 0x04},
	{CCI_REG8(0x3025), 0x03},
	{CCI_REG8(0x3425), 0x01},
	{CCI_REG8(0x3428), 0x01},
	{CCI_REG8(0x3406), 0x08},
	{CCI_REG8(0x3408), 0x03},
	{CCI_REG8(0x340c), 0xff},
	{CCI_REG8(0x340d), 0xff},
	{CCI_REG8(0x031e), 0x09},
	{CCI_REG8(0x3501), 0x08},
	{CCI_REG8(0x3502), 0xe5},
	{CCI_REG8(0x3505), 0x83},
	{CCI_REG8(0x3508), 0x00},
	{CCI_REG8(0x3509), 0x80},
	{CCI_REG8(0x350a), 0x04},
	{CCI_REG8(0x350b), 0x00},
	{CCI_REG8(0x350c), 0x00},
	{CCI_REG8(0x350d), 0x80},
	{CCI_REG8(0x350e), 0x04},
	{CCI_REG8(0x350f), 0x00},
	{CCI_REG8(0x3600), 0x00},
	{CCI_REG8(0x3603), 0x2c},
	{CCI_REG8(0x3605), 0x50},
	{CCI_REG8(0x3609), 0xb5},
	{CCI_REG8(0x3610), 0x39},
	{CCI_REG8(0x360c), 0x01},
	{CCI_REG8(0x3628), 0xa4},
	{CCI_REG8(0x362d), 0x10},
	{CCI_REG8(0x3660), 0x43},
	{CCI_REG8(0x3661), 0x06},
	{CCI_REG8(0x3662), 0x00},
	{CCI_REG8(0x3663), 0x28},
	{CCI_REG8(0x3664), 0x0d},
	{CCI_REG8(0x366a), 0x38},
	{CCI_REG8(0x366b), 0xa0},
	{CCI_REG8(0x366d), 0x00},
	{CCI_REG8(0x366e), 0x00},
	{CCI_REG8(0x3680), 0x00},
	{CCI_REG8(0x36c0), 0x00},
	{CCI_REG8(0x3701), 0x02},
	{CCI_REG8(0x373b), 0x02},
	{CCI_REG8(0x373c), 0x02},
	{CCI_REG8(0x3736), 0x02},
	{CCI_REG8(0x3737), 0x02},
	{CCI_REG8(0x3705), 0x00},
	{CCI_REG8(0x3706), 0x39},
	{CCI_REG8(0x370a), 0x00},
	{CCI_REG8(0x370b), 0x98},
	{CCI_REG8(0x3709), 0x49},
	{CCI_REG8(0x3714), 0x21},
	{CCI_REG8(0x371c), 0x00},
	{CCI_REG8(0x371d), 0x08},
	{CCI_REG8(0x3740), 0x1b},
	{CCI_REG8(0x3741), 0x04},
	{CCI_REG8(0x375e), 0x0b},
	{CCI_REG8(0x3760), 0x10},
	{CCI_REG8(0x3776), 0x10},
	{CCI_REG8(0x3781), 0x02},
	{CCI_REG8(0x3782), 0x04},
	{CCI_REG8(0x3783), 0x02},
	{CCI_REG8(0x3784), 0x08},
	{CCI_REG8(0x3785), 0x08},
	{CCI_REG8(0x3788), 0x01},
	{CCI_REG8(0x3789), 0x01},
	{CCI_REG8(0x3797), 0x04},
	{CCI_REG8(0x3762), 0x11},
	{CCI_REG8(0x3800), 0x00},
	{CCI_REG8(0x3801), 0x00},
	{CCI_REG8(0x3802), 0x00},
	{CCI_REG8(0x3803), 0x0c},
	{CCI_REG8(0x3804), 0x0e},
	{CCI_REG8(0x3805), 0xff},
	{CCI_REG8(0x3806), 0x08},
	{CCI_REG8(0x3807), 0x6f},
	{CCI_REG8(0x3808), 0x0f},
	{CCI_REG8(0x3809), 0x00},
	{CCI_REG8(0x380a), 0x08},
	{CCI_REG8(0x380b), 0x70},
	{CCI_REG8(0x380c), 0x04},
	{CCI_REG8(0x380d), 0x0c},
	{CCI_REG8(0x380e), 0x09},
	{CCI_REG8(0x380f), 0x0a},
	{CCI_REG8(0x3813), 0x10},
	{CCI_REG8(0x3800), 0x00},
	{CCI_REG8(0x3801), 0x00},
	{CCI_REG8(0x3802), 0x00},
	{CCI_REG8(0x3803), 0x00},
	{CCI_REG8(0x3804), 0x0F},
	{CCI_REG8(0x3805), 0x1F},
	{CCI_REG8(0x3806), 0x08},
	{CCI_REG8(0x3807), 0x8F},
	{CCI_REG8(0x3808), 0x0F},
	{CCI_REG8(0x3809), 0x00},
	{CCI_REG8(0x380a), 0x08},
	{CCI_REG8(0x380b), 0x70},
	{CCI_REG8(0x3810), 0x00},
	{CCI_REG8(0x3811), 0x10},
	{CCI_REG8(0x3812), 0x00},
	{CCI_REG8(0x3813), 0x11},
	{CCI_REG8(0x3814), 0x01},
	{CCI_REG8(0x3815), 0x01},
	{CCI_REG8(0x3816), 0x01},
	{CCI_REG8(0x3817), 0x01},
	{CCI_REG8(0x381c), 0x00},
	{CCI_REG8(0x3820), 0x00},
	{CCI_REG8(0x3821), 0x04},
	{CCI_REG8(0x3823), 0x08},
	{CCI_REG8(0x3826), 0x00},
	{CCI_REG8(0x3827), 0x08},
	{CCI_REG8(0x382d), 0x08},
	{CCI_REG8(0x3832), 0x02},
	{CCI_REG8(0x3833), 0x00},
	{CCI_REG8(0x383c), 0x48},
	{CCI_REG8(0x383d), 0x60},
	{CCI_REG8(0x3d85), 0x0b},
	{CCI_REG8(0x3d84), 0x40},
	{CCI_REG8(0x3d8c), 0x63},
	{CCI_REG8(0x3d8d), 0xd7},
	{CCI_REG8(0x4000), 0xf8},
	{CCI_REG8(0x4001), 0x2b},
	{CCI_REG8(0x4004), 0x00},
	{CCI_REG8(0x4005), 0x40},
	{CCI_REG8(0x400a), 0x01},
	{CCI_REG8(0x400f), 0xa0},
	{CCI_REG8(0x4010), 0x12},
	{CCI_REG8(0x4018), 0x00},
	{CCI_REG8(0x4008), 0x02},
	{CCI_REG8(0x4009), 0x0d},
	{CCI_REG8(0x401a), 0x58},
	{CCI_REG8(0x4050), 0x00},
	{CCI_REG8(0x4051), 0x01},
	{CCI_REG8(0x4028), 0x2f},
	{CCI_REG8(0x4052), 0x00},
	{CCI_REG8(0x4053), 0x80},
	{CCI_REG8(0x4054), 0x00},
	{CCI_REG8(0x4055), 0x80},
	{CCI_REG8(0x4056), 0x00},
	{CCI_REG8(0x4057), 0x80},
	{CCI_REG8(0x4058), 0x00},
	{CCI_REG8(0x4059), 0x80},
	{CCI_REG8(0x430b), 0xff},
	{CCI_REG8(0x430c), 0xff},
	{CCI_REG8(0x430d), 0x00},
	{CCI_REG8(0x430e), 0x00},
	{CCI_REG8(0x4501), 0x18},
	{CCI_REG8(0x4502), 0x00},
	{CCI_REG8(0x4643), 0x00},
	{CCI_REG8(0x4640), 0x01},
	{CCI_REG8(0x4641), 0x04},
	{CCI_REG8(0x4800), 0x64},
	{CCI_REG8(0x4809), 0x2b},
	{CCI_REG8(0x4813), 0x90},
	{CCI_REG8(0x4817), 0x04},
	{CCI_REG8(0x4833), 0x18},
	{CCI_REG8(0x4837), 0x0b},
	{CCI_REG8(0x483b), 0x00},
	{CCI_REG8(0x484b), 0x03},
	{CCI_REG8(0x4850), 0x7c},
	{CCI_REG8(0x4852), 0x06},
	{CCI_REG8(0x4856), 0x58},
	{CCI_REG8(0x4857), 0xaa},
	{CCI_REG8(0x4862), 0x0a},
	{CCI_REG8(0x4869), 0x18},
	{CCI_REG8(0x486a), 0xaa},
	{CCI_REG8(0x486e), 0x03},
	{CCI_REG8(0x486f), 0x55},
	{CCI_REG8(0x4875), 0xf0},
	{CCI_REG8(0x5000), 0x89},
	{CCI_REG8(0x5001), 0x42},
	{CCI_REG8(0x5004), 0x40},
	{CCI_REG8(0x5005), 0x00},
	{CCI_REG8(0x5180), 0x00},
	{CCI_REG8(0x5181), 0x10},
	{CCI_REG8(0x580b), 0x03},
	{CCI_REG8(0x4d00), 0x03},
	{CCI_REG8(0x4d01), 0xc9},
	{CCI_REG8(0x4d02), 0xbc},
	{CCI_REG8(0x4d03), 0xc6},
	{CCI_REG8(0x4d04), 0x4a},
	{CCI_REG8(0x4d05), 0x25},
	{CCI_REG8(0x4700), 0x2b},
	{CCI_REG8(0x4e00), 0x2b},
	{CCI_REG8(0x3501), 0x09},
	{CCI_REG8(0x3502), 0x01},
};

const struct ox05b1s_reglist os08a20_reglist_4k60_10b[] = {
	{
		.regs = os08a20_init_setting_4k60_10b,
		.count = ARRAY_SIZE(os08a20_init_setting_4k60_10b),
	},
	{
		/* sentinel */
	}
};

#define OX05B1S_REG_PLL1_CTRL_REG07	CCI_REG8(0x0307)
#define OX05B1S_REG_PLL3_CTRL_REG4A	CCI_REG8(0x034a)
#define OX05B1S_REG_PLL_MONITOR_REG0B	CCI_REG8(0x040b)
#define OX05B1S_REG_PLL_MONITOR_REG0C	CCI_REG8(0x040c)
#define OX05B1S_REG_SC_CMMN_REG09	CCI_REG8(0x3009)
#define OX05B1S_REG_GROUP_HLD_REG19	CCI_REG8(0x3219)
#define OX05B1S_REG_ANA_REG		CCI_REG8(0x3600)
#define OX05B1S_REG_SENSOR_CTRL02	CCI_REG8(0x3702)
#define OX05B1S_REG_TIMING_CTRL		CCI_REG8(0x3800)
#define OX05B1S_REG_MIPI_CORE_REG02	CCI_REG8(0x4802)
#define OX05B1S_REG_MIPI_CORE_REG1B	CCI_REG8(0x481b)
#define OX05B1S_REG_PCLK_PERIOD		CCI_REG8(0x4837)
#define OX05B1S_REG_ISP_REG01		CCI_REG8(0x5001)

/* OX05B1S 2592 x 1944 @30fps GRBG10 */
static const struct cci_reg_sequence ovx5b_init_setting_2592x1944[] = {
	{CCI_REG8(0x0107), 0x01}, /* Reserved */
	{OX05B1S_REG_PLL_MONITOR_REG0B, 0x5c},
	{OX05B1S_REG_PLL_MONITOR_REG0C, 0xcd},
	{OX05B1S_REG_SC_CMMN_REG09, 0x2e},
	{OX05B1S_REG_GROUP_HLD_REG19, 0x08},
	{OX05B1S_REG_ANA_REG + 0x84, 0x6d},
	{OX05B1S_REG_ANA_REG + 0x85, 0x6d},
	{OX05B1S_REG_ANA_REG + 0x86, 0x6d},
	{OX05B1S_REG_ANA_REG + 0x87, 0x6d},
	{OX05B1S_REG_ANA_REG + 0x8c, 0x07},
	{OX05B1S_REG_ANA_REG + 0x8d, 0x07},
	{OX05B1S_REG_ANA_REG + 0x8e, 0x07},
	{OX05B1S_REG_ANA_REG + 0x8f, 0x00},
	{OX05B1S_REG_ANA_REG + 0x90, 0x04},
	{OX05B1S_REG_ANA_REG + 0x91, 0x04},
	{OX05B1S_REG_ANA_REG + 0x92, 0x04},
	{OX05B1S_REG_ANA_REG + 0x93, 0x04},
	{OX05B1S_REG_ANA_REG + 0x98, 0x00},
	{OX05B1S_REG_ANA_REG + 0xa0, 0x05},
	{OX05B1S_REG_ANA_REG + 0xa2, 0x16},
	{OX05B1S_REG_ANA_REG + 0xa3, 0x03},
	{OX05B1S_REG_ANA_REG + 0xa4, 0x07},
	{OX05B1S_REG_ANA_REG + 0xa5, 0x24},
	{OX05B1S_REG_ANA_REG + 0xe3, 0x09},
	{OX05B1S_REG_SENSOR_CTRL02, 0x0a},
	{OX05B1S_REG_TIMING_CTRL + 0x21, 0x04}, /* mirror */
	{OX05B1S_REG_TIMING_CTRL + 0x22, 0x10},
	{OX05B1S_REG_TIMING_CTRL + 0x2b, 0x03},
	{OX05B1S_REG_TIMING_CTRL + 0x66, 0x10},
	{OX05B1S_REG_TIMING_CTRL + 0x6c, 0x46},
	{OX05B1S_REG_TIMING_CTRL + 0x6d, 0x08},
	{OX05B1S_REG_TIMING_CTRL + 0x6e, 0x7b},
	{OX05B1S_REG_MIPI_CORE_REG02, 0x00},
	{OX05B1S_REG_MIPI_CORE_REG1B, 0x3c},
	{OX05B1S_REG_ISP_REG01, 0x00},

	/* MIPI datarate 640 Mbps */
	{OX05B1S_REG_PLL1_CTRL_REG07, 0x02},
	{OX05B1S_REG_PLL3_CTRL_REG4A, 0x05},
	{OX05B1S_REG_PCLK_PERIOD, 0x19},
};

/*
 * OX05B1S MIPI datarate 960 Mbps, enough for:
 * 2592 * 1944 * 30fps * 10bit / 1024 / 1024 / 4 lanes = ~360 Mbps per lane
 * 2592 * 1944 * 60fps * 10bit / 1024 / 1024 / 4 lanes = ~720 Mbps per lane
 * 2592 * 1944 * 30fps * 10bit / 1024 / 1024 / 2 lanes = ~720 Mbps per lane
 * with a 1.33 factor for mipi overhead: ~958 Mbps for 30fps on 2 lanes
 */
static const struct cci_reg_sequence ovx5b_init_setting_960Mbps[] = {
	{OX05B1S_REG_PLL1_CTRL_REG07, 0x01},
	{OX05B1S_REG_PLL3_CTRL_REG4A, 0x03},
	{OX05B1S_REG_PCLK_PERIOD, 0x10},
};

const struct ox05b1s_reglist ox05b1s_reglist_2592x1944[] = {
	{
		.regs = ovx5b_init_setting_2592x1944,
		.count = ARRAY_SIZE(ovx5b_init_setting_2592x1944),
	},
	{
		.regs = ovx5b_init_setting_960Mbps,
		.count = ARRAY_SIZE(ovx5b_init_setting_960Mbps),
	},
	{
		/* sentinel */
	}
};
