...
static void mxc_isi_async_timeout(struct work_struct *ptr)
{
	...
	list_for_each_entry_safe(sd, tmp, &v4l2_dev->subdevs, list) {
		if (sd && sd->fwnode && sd->dev) {
			struct device_node *np, *port, *ep, *remote;

			np = to_of_node(sd->fwnode);

			if (np) {
				bool need_to_unload = false;

				for_each_child_of_node(np, port) {
					if (!of_node_name_eq(port, "ports") && !of_node_name_eq(port, "port"))
						continue;
					...
					for_each_child_of_node(port, ep) {
						...
						remote = of_graph_get_remote_endpoint(ep->child);
						if (!remote || !remote->parent || !remote->parent->parent || !remote->parent->parent->parent)
							continue;

						struct i2c_client *client = of_find_i2c_device_by_node(remote->parent->parent->parent);

						if (client) {
							if (!client->dev.driver) {
								dev_warn(isi->dev, "%s i2c driver failed to load\n", sd->name);
								need_to_unload = true;
							}
						}
					}
				...
				if (need_to_unload) {
					struct platform_device *pdev = to_platform_device(sd->dev);

					for (int i = 0; i < isi->pdata->num_channels; ++i)
						if (&isi->pipes[i].sd == sd)
							mxc_isi_video_unregister(&isi->pipes[i]);
	...
	if (list_empty(&isi->notifier.waiting_list)) {
		bool all_subdevs_ready = true;

		list_for_each_entry_safe(sd, tmp, &v4l2_dev->subdevs, list) {
			struct v4l2_async_connection *asc;

			if (sd->asc_list.next) {
				list_for_each_entry(asc, &sd->asc_list, asc_subdev_entry) {
					if (asc && asc->notifier) {
						if (!list_empty(&asc->notifier->waiting_list))
							all_subdevs_ready = false;
					}
				}
			}
		}

		if (all_subdevs_ready) {
